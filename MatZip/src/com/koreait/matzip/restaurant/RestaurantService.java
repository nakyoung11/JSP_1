...
public class RestaurantService {
	...
	public int addRecMenus(HttpServletRequest request) {
		String savePath = request.getServletContext().getRealPath("/res/img/restaurant");
			/* context내의 절대주소값(서버(톰캣)이 돌아가고 있는 )이 넘어감 그후에 +savePath+/temp 
								"/"를 사용하면  모든 운영체제에서 사용가능 */
		System.out.println(savePath);
		String tempPath = savePath + "/tepm";//임시로 이미지를 저장~
		...
		try {
			...
			if(menu_nmArr == null || menu_priceArr == null){System.out.println(menu_nmArr[0]);	System.out.println(menu_priceArr[0]); return i_rest;}
