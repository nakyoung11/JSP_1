...
public class RestaurantDAO {
	public int insrestRegProc(RestaurantVO param) {
	...
	public RestaurantDomain selRest(RestaurantVO param) {
		...
		String sql=  " SELECT  A.nm, A.addr, A.i_user, A.hits AS cntHits, A.i_rest "  
				...
				+" AND B.i_m =1 " 
				+" LEFT JOIN( "
				+"	SELECT 1 AS i_rest, COUNT(i_rest) AS cnt  " 
		...
		JdbcTemplate.executeQuery(sql, new JdbcSelectInterface() {
			...
			public void executeQuery(ResultSet rs) throws SQLException {
				if(rs.next()) {
					...
					vo.setCd_category_nm(rs.getString("cd_category_nm"));
